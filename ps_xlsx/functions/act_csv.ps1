        ...
        foreach($f in $ck_f){
                        ...
                        ・出力ファイル存在確認
                            ...
                            2)出力ファイルが存在しない場合
                                ...
                                2)出力ファイルが存在しない場合
                                    ...
                                    2.Excelブックオープン処理
                                    ※Whileループにてオープン処理が起動(Ready)になるまで待機
                                    3.Excelブック内ワークシートへ接続
                                    4.Excelワークシート内最終行取得
                                    5.Excelワークシート内最終列取得
                                    6.Excelワークシート最終列に1を加算
                                    7.Excelワークシート最終列に追記処理(for文)
                                    8.Excelファイルの保存
                                    9.Excelオブジェクト終了
                                    10.Excelオブジェクト開放処理
                                    ②入力ファイル名が変数指定ファイル名と同一でない場合
            ...
            #>
                ...
                if(Test-Path $dst_fs){
                    ...
                    ## Excelファイル整形：事前処理
                    # ファイルコピー処理
                    Copy-Item $xlsx_file_path $dst_fs
                    ...
                    if(Test-Path $dst_fs){
                        ...
                        # Excelファイルオープン処理
                        $book = $excel.Workbooks.Open($dst_fs)

                        # WorkSheet接続
                        while(-not $excel.Ready){
                            $l_msg = "Excelファイルオープン準備"
                            cm_msg $l_msg
                            sleep 1
                        }
                        $sh = $excel.Worksheets.Item(1)
                        ...
                        # WorkSheet最終列へのアイテム挿入
                        for($i = 1; $i -eq $Cols; $i++){
                            if($i -eq "1"){
                            # カラムヘッダー生成
                                $sh.Range($sh.Cells.Item(1,$Row)) = "日付"
                            }
                            else{
                            # Value生成
                                $sh.Range($sh.Cells.Item($i,$Row)) = $YYMM
                            }
                        }
